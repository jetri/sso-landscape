...
**Requirements:** There is no permission literally named "OBO." The middle-tier app registration must have **delegated permissions** on the downstream API, with **prior user or admin consent** already granted for those scopes. On token exchange, the middle tier must request the downstream resource's `/.default` scope—an Entra requirement for OBO that returns all statically consented delegated permissions for that registration on the resource. Do not request individual downstream scopes on the OBO call; OBO cannot prompt the user for new permissions mid-flow. The downstream API validates the new token's `aud`, scopes, and user identity claims as usual. OBO is an Entra / Microsoft identity platform pattern—not a generic OAuth grant; other IdPs may use different token-exchange mechanisms.
